...
namespace Microsoft.FSharp.Text.Lexing 
    ...
    type LexBufferFiller = 
        { fillSync : (LexBuffer -> unit) option
          fillAsync : (LexBuffer -> Async<unit>) option } 
        
    and [<Sealed>]
        /// Input buffers consumed by lexers generated by <c>fslex.exe </c>
        LexBuffer(filler: LexBufferFiller) as this = 
        ...
        static member FromReadFunctions (syncRead : (char[] * int * int -> int) option, asyncRead : (char[] * int * int -> Async<int>) option) : LexBuffer = 
        ...
        static member FromFunction (f : char[] * int * int -> int) : LexBuffer =  LexBuffer.FromReadFunctions(Some(f),None)
        /// Create a lex buffer that asynchronously reads character or byte inputs by using the given function
        static member FromAsyncFunction (f : char[] * int * int -> Async<int>) : LexBuffer =  LexBuffer.FromReadFunctions(None,Some(f))
        ...
        // A full type signature is required on this method because it is used at more specific types within its own scope
        static member FromArray (s: char[]) : LexBuffer = 
            let lexBuffer = 
                new LexBuffer 
                    { fillSync = Some (fun _ -> ()); 
