  ...
  if log then
      let actionMatrix =
          Array.map (fun (row : ActionTableRow) ->
            match row with
            | ImmediateAction _ -> Array.create preprocessed.Terminals.Length None
            | LookaheadActions ary ->
                Array.map (fun (action : Action) ->
                    match action with
                    | Error -> None
                    | _ -> Some 0) ary) compiled.ActionTable
      
      let actionHashtable = Hashtable.create None actionMatrix
      Hashtable.outputHashtableStat actionMatrix actionHashtable
      Hashtable.outputHashtableImage "actionTableImage.png" actionHashtable

      let gotoHashtable = Hashtable.create None compiled.GotoTable
      Hashtable.outputHashtableStat compiled.GotoTable gotoHashtable
      Hashtable.outputHashtableImage "gotoTableImage.png" gotoHashtable
